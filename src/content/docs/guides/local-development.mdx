...
This guide is written for **Recogito Studio v1.10.0** and later. If you're working with an earlier version, some steps may differ.
:::
...
- [**Trigger.dev**](https://trigger.dev/docs/self-hosting/overview): Required for background jobs like project import/export and the NER plugin. After setting `TRIGGER_PROJECT_ID` and `TRIGGER_SECRET_KEY` (Phase 2), run `npx trigger.dev@latest dev` in the client repo to process tasks locally; see the [client README](https://github.com/recogito/recogito-client?tab=readme-ov-file#triggerdev) for details.
...
2. **Create environment file**
   ...
   ```bash
      # App base URL. Required by the Node adapter (used for Astro's allowedDomains).
      SITE_URL="http://localhost:4321"

      # Optional – plugins to register, as a comma- or space-separated list of npm
      # package names (they must also be installed; see the note below the block).
      # INSTALLED_PLUGINS="@recogito/plugin-geotagging"
      ...
      # Optional – Require users to accept a EULA before uploading documents
      # PUBLIC_EULA_URL="https://example.com/eula"
      ...
      # TRIGGER_SERVER_URL is only needed for a self-hosted Trigger.dev instance
   ...
   To use [plugins](/guides/plugins/), install the packages and list them in `INSTALLED_PLUGINS`:
...
:::note[Node adapter requires `SITE_URL`]
As of v1.9.6, the Node adapter reads `SITE_URL` to configure Astro's [`allowedDomains`](https://docs.astro.build/en/reference/configuration-reference/#securityalloweddomains). Set `SITE_URL` to your deployment URL (or `http://localhost:4321` in development), or requests may be rejected.
:::
...
- [Configuration Tool Reference](/reference/config-tool/) - Roles, groups, branding, and authentication
