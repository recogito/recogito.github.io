...
1. **Install** (build time) — declare the plugin in the `INSTALLED_PLUGINS` environment variable and build the client. This installs the package, compiles it into your instance, and makes it _available_ to every project. It's done once per instance by whoever builds and deploys the client.
