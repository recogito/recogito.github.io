...
2. Run the development process in your project root folder:
```bash
npm install
npm run dev
```
...
The `.dev/` harness mounts your extension in isolation, which is ideal for iterating on the component itself. But it renders the extension in a bare container—it does **not** reproduce the surrounding Recogito Studio UI. Behavior that depends on that context (e.g. layout inside the annotation editor, styles inherited from the client) will only show up in the real client. For those cases, test your unpublished, local plugin against a full [`recogito-client`](/guides/local-development/#phase-2-client-application-setup-recogito-client) checkout:

<Steps>

1. **Build the plugin.** The client imports the compiled `dist/`, not your source, so build first (and rebuild after every change):

   ```sh
   npm run build
   ```

   :::tip[Watch mode and CSS]
   `tsc --watch` recompiles TypeScript on save, but it does **not** re-run `copy-files`, so changes to `.css` (or other copied assets) won't be picked up. After editing styles, re-run the full `npm run build`.
   :::

2. **Add the checkout as a local `file:` dependency** so the client resolves it by package name. In the client's `package.json`, point the dependency at your plugin directory, then `npm install`:

   ```jsonc
   "dependencies": {
     // ...
     "@performant/plugin-hello-world": "file:../plugin-hello-world"
   }
   ```

   :::note
   This symlinks the plugin directory, so edits are picked up live. Don't commit this dependency line to the client repo.
   :::

3. **Register it.** List the package name in `INSTALLED_PLUGINS` in the client's `.env`, then restart the dev server:

   ```sh
   INSTALLED_PLUGINS="@performant/plugin-hello-world"
   ```

4. **Run the client** and [enable the plugin](/guides/plugins/#enabling-an-installed-plugin) per project from **Project Settings → Plugins**.
...
An extension's `extension_point` decides where its component mounts. The available points are:

- `admin` — the instance admin settings area
- `project:document-actions` — the document card actions menu
- `annotation:<view>:annotation-editor` — inside the annotation editor
- `annotation:<view>:annotator` — alongside the annotator instance
- `annotation:<view>:toolbar` — the annotation view toolbar
- `annotation:<view>:taglist` — the annotation tag list
...
Once your plugin is publishable (to the npm registry, or installable from a Git URL), it's added to a Recogito Studio instance by listing its package name in the `INSTALLED_PLUGINS` environment variable and rebuilding the client. A Project Admin then enables it per project from **Project Settings → Plugins**.

See the [Plugins guide](/guides/plugins/) for the full install-and-enable flow.

## Component Styling

The Recogito Studio Client utilizes <a href="https://www.radix-ui.com/">Radix UI</a> primitives for building accessible and consistent interface components. We recommend that plugin developers also adopt Radix when developing extension components.​
